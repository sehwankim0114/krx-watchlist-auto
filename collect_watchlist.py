...
def kr_tick_round(x: float) -> Optional[int]:
    if x is None or pd.isna(x) or x <= 0:
        return None
    # KRX 호가단위는 시장/가격대별 세부 규칙이 있지만 관종표용 기준가 산정에는 보수적 근사값을 사용합니다.
    if x < 2000:
        unit = 1
    elif x < 5000:
        unit = 5
...
def summarize_history(
    ...
    if hist is None or hist.empty:
        return {
            "name": name, "ticker": ticker, "country": country, "currency": currency, "exchange": exchange,
            "status": "NO_DATA", "error": "history empty"
        }
    ...
    if "close" not in df.columns:
        return {
            "name": name, "ticker": ticker, "country": country, "currency": currency, "exchange": exchange,
            "status": "NO_CLOSE", "error": f"columns={list(hist.columns)}"
        }
    ...
    if df.empty:
        return {
            "name": name, "ticker": ticker, "country": country, "currency": currency, "exchange": exchange,
            "status": "NO_CLOSE_ROWS", "error": "close rows empty"
        }
    ...
    if price_range > 0:
        ...
        target1 = min(current + avg_abs_move * 2.2 if not pd.isna(avg_abs_move) else current * 1.08,
                      low + price_range * 0.78)
        stop = max(low * 0.97, current - avg_abs_move * 3.0 if not pd.isna(avg_abs_move) else current * 0.90)
    else:
        ...
        try:
            if country == "KR":
                ...
                if hist is not None and not hist.empty:
                    ...
                    raw_frames.append(h.reset_index(names="date"))
                ...
                s["source_used"] = "yfinance"
                if hist is not None and not hist.empty:
                    h = hist.copy()
                    h["name"] = name
                    h["ticker"] = ticker
                    h["country"] = country
                    h["source_used"] = "yfinance"
                    raw_frames.append(h.reset_index(names="date"))
            else:
                s = {
                    "name": name, "ticker": ticker, "country": country, "currency": currency, "exchange": exchange,
                    "status": "UNSUPPORTED_COUNTRY", "error": f"country={country}"
                }

            summaries.append(s)
            print(f"[OK] {name} {ticker}: {s.get('status')} {s.get('last_date','')}")
        except Exception as e:
