...
1. `getApiStatus` 호출
2. `getStockTableRules` 호출
3. 요청어에 맞는 **본표 Action 하나** 호출
4. 상태파일과 본표의 `build_id`, 규칙 해시, 행 수 대조
5. 검증 정상일 때만 표 작성
...
- `presentation_policy.default_output_mode=single_main_table`
- `presentation_policy.separate_recommendation_table_default=false`
- `presentation_policy.recommendation_markings_embedded_in_main_table=true`
- `presentation_policy.duplicate_rows_across_main_and_shortlist_tables=false`
