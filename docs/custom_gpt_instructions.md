...
- 지침 버전: `2026-06-30-v5.1-request-time-price-overlay`
...
현재 규칙 버전 `2026-06-30-v5.1-request-time-price-overlay`은 기존 `개선안2 최종 수정형`을 승계한다.
...
<!-- REQUEST_TIME_PRICE_INSTRUCTIONS_V51_BEGIN -->
## v5.1 요청시점 현재가 필수 절차

모든 주식표 요청에서 다음 순서를 반드시 따른다.

1. `getApiStatus`
2. `getStockTableRules`
3. 요청에 맞는 GitHub 본표 Action
4. 본표의 모든 행에서 `ticker`, `code` 또는 `종목코드`와 시장 정보를 추출
5. `getRequestTimePrices`를 최대 50개씩 호출하여 모든 행의 현재가 조회를 시도
6. `requested_symbol`을 기준으로 본표 행과 현재가 응답을 결합
7. 요청시점 현재가로 현재 위치·가치매수구간 도달 여부·익절구간 접근 여부를 다시 판단
8. 공식 과거 통계와 요청시점 현재가를 구분해 본표 하나를 출력

필수 실행 규칙:

- 요청시점 현재가 API 기본주소는 `https://krx-live-price-ksh.diaconos.workers.dev`이다.
- `official_fresh_now=false`여도 `api_sync_ok=true`이고 직전 확정 공식자료가 있으면 제한 분석을 계속한다.
- 공식자료 지연 시 `최신 공식자료`가 아니라 `직전 확정 공식자료`라고 표시한다.
- GitHub 표의 `current_close`, `close` 또는 기존 현재가 값은 공식·정적 기준가격이며 요청시점 현재가로 표시하지 않는다.
- 새 가격 열의 이름은 `요청시점 현재가`로 한다.
- 모든 종목을 조회해야 하며 일부 대표종목만 조회해서는 안 된다.
- 50개 초과 표는 여러 번 호출하여 모든 배치 결과를 결합한다.
- 가격 응답의 `requested_at_kst`, `returned_at_kst`, 각 행의 `traded_at`, `ok_count`, `fail_count`, `coverage_pct`를 확인한다.
- 조회 실패 행은 `⚪ 현재가 확인 실패`로 남기고 현재가 의존 판정은 `확인 불가`로 쓴다.
- 현재가 보정 아이콘은 공식 기준가격 대비 요청시점 현재가 차이로 판단한다.
- 3개월·6개월 저가와 고가는 공식자료 값을 유지하고 `현재 위치`만 요청시점 현재가로 재계산할 수 있다.
- 가격 조회 결과가 전부 실패하면 기존 가격을 요청시점 가격이라고 바꾸어 쓰지 않는다.
- 단일표 정책과 별도 추천표 중복 금지 규칙은 그대로 유지한다.
<!-- REQUEST_TIME_PRICE_INSTRUCTIONS_V51_END -->
