...
- latest/kosdaq_1m_recommend_5_latest.csv
- latest/kosdaq_1m_run_log_latest.txt
- latest/kosdaq_1m_status_latest.json

핵심 원칙
- 1개월 저가·고가·변동폭·현재위치를 실제 1개월 필드로 계산한다.
- 기존 코피표의 3개월 위치를 대신 사용하지 않는다.
- 후보 10개 본표 안에 추천 5개를 ✅로 표시한다.
- 별도 추천 5개 파일은 명시적 요청용 내부 산출물이다.
...
SCRIPT_VERSION = "kosdaq_one_month.py v1.0.0-complete-one-month-fields-only"
POLICY_VERSION = "2026-07-03-v6.0-kosdaq-one-month-value-candidates-v1"
...
def score_one_month_candidate(row: pd.Series) -> Dict[str, Any]:
    ...
    hard_red = (
        ...
        or (not pd.isna(return_1m) and return_1m > 30)
    )
    warning = (
        liquidity_burden
        or overheat
...
def build_candidates(
    ...
    if summary is None or summary.empty:
        raise ValueError("KOSDAQ summary is empty")

    missing = sorted(REQUIRED_INPUT_COLUMNS - set(summary.columns))
    if missing:
        raise ValueError(
            "KOSDAQ summary missing one-month columns: "
            + ",".join(missing)
        )
    ...
    if len(frame) < candidate_n:
        raise ValueError(
            "Complete KOSDAQ one-month rows are insufficient: "
    ...
    # 추천 5개는 반드시 후보 10개 안에서만 선정한다.
    # 전체 종목에서 따로 추천을 뽑으면 후보표 밖 종목이
    # 추천으로 들어갈 수 있으므로 top을 유일한 추천 모집단으로 사용한다.
    eligible = top[
...
def write_outputs(
    ...
    candidate_path = (
        output_dir / "kosdaq_1m_candidates_10_latest.csv"
    )
    recommend_path = (
        output_dir / "kosdaq_1m_recommend_5_latest.csv"
    )
    log_path = output_dir / "kosdaq_1m_run_log_latest.txt"
    status_path = output_dir / "kosdaq_1m_status_latest.json"
    ...
    log_lines = [
        ...
        "TABLE_ID=kosdaq_1m",
        "DISPLAY_NAME=코닥표1개월",
        "ANALYSIS_SCOPE=KOSDAQ_ALL_RECENT_ONE_MONTH",
        "POSITION_PERIOD=1개월",
        ...
        "CANDIDATE_EXPECTED=10",
        "RECOMMEND_EXPECTED=5",
...
def run_self_test() -> int:
    ...
    assert metadata["recommend_source"] == "candidate_top_10"
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        expected = [
            ...
            "kosdaq_1m_run_log_latest.txt",
            "kosdaq_1m_status_latest.json",
        ]
        for name in expected:
            assert (output_dir / name).exists()

        log_text = (
            output_dir / "kosdaq_1m_run_log_latest.txt"
        ).read_text(encoding="utf-8")
        assert "CANDIDATE_ROWS=10" in log_text
        assert "RECOMMEND_ROWS=5" in log_text
    ...
    print(
        ...
        "score_0_100,"
        "recommend_subset,"
        "recommend_selected_only_from_top10,"
        "single_table_policy,"
        "output_files"
    )
    return 0
...
def main() -> int:
    ...
    print("KOSDAQ_ONE_MONTH_STATUS=OK")
    print(
    ...
    print("RECOMMEND_SOURCE=candidate_top_10")
