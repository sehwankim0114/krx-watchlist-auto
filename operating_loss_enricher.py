...
"""
영업손실 자동판별 보강기
v1.0_operating_loss_enricher

생성/갱신 파일
- latest/operating_loss_cache_latest.csv
- latest/operating_loss_run_log_latest.txt

기능
- OpenDART 고유번호(corp_code)와 단일회사 주요계정 API를 이용해 최근 확정 보고서의 영업이익을 확인한다.
- 기존 latest/*.csv 표 파일에 아래 컬럼을 추가/갱신한다.
  - operating_profit
...
SCRIPT_NAME = "operating_loss_enricher.py v1.0_operating_loss_enricher"
...
def get_ticker_column(df: pd.DataFrame) -> Optional[str]:
    for col in ["ticker", "code", "종목코드", "단축코드"]:
    ...
    if cache_path.exists():
        try:
            ...
            if (now_kst() - mtime).total_seconds() < 86400:
                cached = pd.read_csv(cache_path, encoding="utf-8-sig", dtype=str).fillna("")
                mapping = {
...
def fetch_operating_profit_for_ticker(
    ...
    if not corp_code:
        return {
            ...
            "operating_loss_flag": "",
            "operating_loss_basis": "",
            "operating_loss_source_status": "NO_CORP_CODE",
            "account_nm": "",
            "fs_div": "",
            "sj_div": "",
            "bsns_year": "",
    ...
    return {
        ...
        "operating_loss_flag": "",
        "operating_loss_basis": "",
        "operating_loss_source_status": last_status or "NO_FINANCIAL_DATA",
        "account_nm": "",
        "fs_div": "",
        "sj_div": "",
        "bsns_year": "",
...
def read_cache(cache_path: Path, target_key: str) -> Dict[str, Dict[str, Any]]:
    ...
    for _, row in df.iterrows():
        ...
        record = row.to_dict()
        if str(record.get("operating_loss_flag", "")).lower() in {"true", "1", "yes"}:
            record["operating_loss_flag"] = True
        elif str(record.get("operating_loss_flag", "")).lower() in {"false", "0", "no"}:
...
def main() -> None:
    ...
    if not api_key:
        ...
        for ticker in tickers:
            result_map[ticker] = {
                ...
                "operating_loss_flag": "",
                "operating_loss_basis": "",
                "operating_loss_source_status": "NO_DART_API_KEY",
                "account_nm": "",
                "fs_div": "",
                "sj_div": "",
                "bsns_year": "",
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    result = {
                        ...
                        "operating_loss_flag": "",
                        "operating_loss_basis": "",
                        "operating_loss_source_status": f"UNHANDLED_ERROR_{type(exc).__name__}",
                        "account_nm": "",
                        "fs_div": "",
                        "sj_div": "",
                        "bsns_year": "",
    ...
    if not cache_df.empty:
        loss_count = int(cache_df["operating_loss_flag"].astype(str).str.lower().isin(["true", "1", "yes"]).sum())
