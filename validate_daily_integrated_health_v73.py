...
def validate_local_repository(
    ...
    for spec in TABLE_SPECS:
        ...
        if file_bytes <= MAX_WATCHLIST_BYTES:
            report.pass_check(
                f"{table_id}_payload_size",
                f"{table_id} 응답 크기가 안전 범위입니다.",
                {"bytes": file_bytes, "limit": MAX_WATCHLIST_BYTES},
            )
        else:
            report.fail(
                f"{table_id}_payload_size",
                f"{table_id} 응답 크기가 제한을 넘었습니다.",
                {"bytes": file_bytes, "limit": MAX_WATCHLIST_BYTES},
            )
